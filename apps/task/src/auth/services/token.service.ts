...
export class TokenService {
  ...
  async refreshTokens(sub: number, jti: UUID): Promise<SignedTokenPair> {
    ...
    if (!storedToken) {
      throw new InternalServerErrorException(
        'The provided token is not stored',
      );
    }
    ...
    return await this.generateAndStoreTokenPair(sub);
  }
  ...
  async generateAndStoreTokenPair(sub: number): Promise<SignedTokenPair> {
    const accessToken = await this.signAccess(sub);
    const refreshToken = await this.signAndStoreRefresh(sub);
    return { accessToken, refreshToken };
  }

  private async signAccess(sub: number): Promise<string> {
